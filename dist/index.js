...
style.appendChild(document.createComment(name + ' ' + "1.3.0"));
...
function colorPicker(source) {
  ...
  if (initialised && inDOM) {
    ...
    var setCurrent = function setCurrent() {
      ...
      Array.from(popup.querySelectorAll('.' + current)).forEach(function (elm) {
        return elm.classList.remove(current);
      });
      (_popup$querySelector = popup.querySelector("button[value=\"".concat(colorInst.hex, "\"]"))) === null || _popup$querySelector === void 0 ? void 0 : _popup$querySelector.classList.add(current);
    };
    /**
    ...
    var hasDatalist = !!(source !== null && source !== void 0 && source.hasAttribute('list'));
    ...
    var list = hasDatalist && append(popup, 'ul');
    hasDatalist && Array.from(source.list.options).forEach(function (option) {
      var li = append(list, 'li');
      var button = append(li, 'button');
    ...
    var ruleColorcolor = getRule("".concat(baseRule, " div:first-of-type {}"));
    var ruleColor = getRule("".concat(baseRule, " div:first-of-type:after {}"));
